...
export function Hero() {
  ...
  const open = () => {
    ...
    tl
      ...
      // 4. SAVE THE DATE card rises and settles slightly right of center
      .to(cardRef.current, {
        autoAlpha: 1, y: -260, x: 30, rotate: 6,
        duration: 0.45, ease: "power2.out",
      }, "-=0.25")

      // 5. Photo 1 (LEFT) — rises with the rest of the group shifted right
      .to(sheet1Ref.current, {
        autoAlpha: 1, y: -200, x: 30, rotate: -8, scale: 1,
        duration: 0.4, ease: "power2.out",
      }, "-=0.1")

      // 6. Photo 2 (RIGHT) — same right shift to keep the group together
